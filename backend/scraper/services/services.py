...
async def initialize_supabase_db():
    supabase: Client = create_client(get_settings().supabase_url, get_settings().supabase_service_key)
    return supabase
...
async def get_google_search_results(query, filters, num_results):
    ...
    url = f"https://www.googleapis.com/customsearch/v1?key={get_settings().CSE_API_KEY}&cx={get_settings().ENGINE_ID}&q={query}&num={num_results}"
    urls=[]
    ...
    if data:
        ...
        raise HTTPException(status_code=404, detail="Google search time out!")
    return urls
...
async def scrape_data_to_json(query: CreateQuery, session: AsyncSession):
    ...
    for url in urls:
        ...
        contact_info_query = CreateLeads(
            email = res['email'] if res['email'] != 'None' or res['email'] != 'Null' else None,
            phone = res['phone_number'] if res['phone_number'] != 'None' or res['phone_number'] != 'Null' else None,
            url = url
        ) # type: ignore
